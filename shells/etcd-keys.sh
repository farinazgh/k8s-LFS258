kubectl exec -it etcd-ip-172-31-40-74 -n kube-system -- /bin/sh
...
etcdctl get "" --prefix --keys-only --insecure-transport=true
...
kubectl -n kube-system exec -it etcd-ip-172-31-40-74  -- /bin/sh -c " ETCDCTL_API=3 ETCDCTL_CACERT=/etc/kubernetes/pki/etcd/ca.crt ETCDCTL_CERT=/etc/kubernetes/pki/etcd/server.crt ETCDCTL_KEY=/etc/kubernetes/pki/etcd/server.key etcdctl --endpoints=https://127.0.0.1:2379 member list"
#85b662448f547898, started, ip-172-31-40-74, https://172.31.40.74:2380, https://172.31.40.74:2379, false
#ca.crt ca.key healthcheck-client.crt healthcheck-client.key peer.crt peer.key server.crt server.key


kubectl -n kube-system exec -it etcd-ip-172-31-40-74  -- /bin/sh -c " ETCDCTL_API=3  ETCDCTL_CACERT=/etc/kubernetes/pki/etcd/ca.crt ETCDCTL_CERT=/etc/kubernetes/pki/etcd/server.crt ETCDCTL_KEY=/etc/kubernetes/pki/etcd/server.keyetcdctl --endpoints=https://127.0.0.1:2379 get "" --prefix --keys-only"
...
kubectl exec -it etcd-ip-172-31-40-74 -n kube-system -- /bin/sh                                                                                                                         export ETCDCTL_API=3
export ETCDCTL_CACERT=/etc/kubernetes/pki/etcd/ca.crt
export ETCDCTL_CERT=/etc/kubernetes/pki/etcd/server.crt
export ETCDCTL_KEY=/etc/kubernetes/pki/etcd/server.key
etcdctl --endpoints=https://127.0.0.1:2379 get "" --prefix --keys-only
